        ...
        if self.round_number == 1:
            if self.player.participant.treat == 'same':
                if self.player.id_in_group == 1:
                    ...
                    if not self.case == 'no_partner_round2_recipient':
            ...
            if self.player.participant.treat == 'same':
                if self.player.id_in_group == 1:
                    if self.case == 'dictator_timeout_2':
                        ...
                        yield Submission(PlayerA_Offer, timeout_happened=True)
                    if not self.case == 'no_partner_round2_recipient':
                        yield PlayerA_Offer, dict(offer=cu(60))
                        expect(self.player.payoff, Constants.endowment - self.player.offer)
                        yield PlayerA_CBG, dict(confl=3, bad=5, good=7)
                        expect(self.player.confl, 3)
                        expect(self.player.bad, 5)
                    ...
                    if not self.case == 'no_partner_round2_dictator':
                        ...
                        if self.case == 'basic':
                            expect(self.player.participant.payoff_plus_participation_fee(),
                                   self.player.participant.payoff.to_real_world_currency(self.session) + self.player.session.config['participation_fee'])